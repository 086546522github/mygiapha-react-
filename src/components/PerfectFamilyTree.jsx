...
import './PerfectFamilyTree.css';

const PerfectFamilyTree = ({ user, onNavigate }) => {
  ...
  const loadData = () => {
    const userKey = `perfect_tree_${user?.id || 'guest'}`;
  ...
  const browseBranch = (memberId) => {
    ...
    const branchMembers = [member, ...(spouse ? [spouse] : []), ...children];
  ...
  return (
    <div className="perfect-family-tree">
      {/* Header */}
...
export default PerfectFamilyTree;
